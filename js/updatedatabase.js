...
function checkIfRequireUpdate(callback) {
	...
	if(loadItem("lastKnownUpdatedID") == null || loadItem("lastKnownUpdatedID") == 0) {
		//If this is true, likely first time this site has be visited. Initalize database. 
		//Otherwise, another potential problem is a server issue (lastKnownUpdatedID from server always = 0)
		callback(true);
		return true;
	} else {
		$.ajax("/getlastid.php", {
