...
function handleAbility(e, ability) {
    ...
    for(var i = 0; i < extraAbility.length; i++) {
        ...
        queryString = queryString + "SELECT Explanation FROM Searches WHERE Name = " + extraAbility[i] + " AND ID = 7";
        firstTime = false;
    }
    for(i = 0; i < extraConditions.length; i++) {
        ...
        queryString = queryString + "SELECT Explanation FROM Searches WHERE Name = " + extraConditions[i] + " AND ID = 5";
        firstTime = false;
    }
    for(i = 0; i < extraMechanics.length; i++) {
        if(!firstTime) {
            queryString = queryString = " UNION ";
        }
        queryString = queryString + "SELECT Explanation FROM Searches WHERE Name = " + extraMechanics[i] + " AND ID = 6";
        firstTime = false;
    }
